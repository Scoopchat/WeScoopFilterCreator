...
func loadReferenceNode() -> SCNNode {
    if referenceNode == nil {
        ...
        do{
            let node =  try SCNReferenceNode(url: url!)!
            node.load()
            referenceNode = node
        }
        catch{
...
func loadAlternateTextureCoordinates( geometry : SCNGeometry ) -> SCNGeometry? {
    ...
    if alternateTextureCoordinates.count == 0 {
        ...
        if let uvPath = Bundle.main.url(forResource: "daz3duv2", withExtension: "plist", subdirectory: nil) {
            
            let allValues =  NSArray(contentsOfFile: uvPath.path) as? [Double]
            
            var newUVCoords = [CGPoint]()
            
            let pairs = (allValues?.count ?? 0)/2
            for i in 0..<pairs {
                let offset = i * 2
                guard let x = allValues?[offset] else { return nil }
                guard let y = allValues?[offset+1] else { return nil}
